...
- **Dependencies:** manage via each package's `pyproject.toml`. The
  ...
  xqsa / xqffi members carry their own. Never modify the dev-dep
...
- **Virtual environment:** always use the workspace `.venv/` managed
  ...
  maturin-built `xqffi` extension is picked up without a manual
  activation step.
...
- **Rust ↔ Python bindings (`xqffi/`):** the pyo3 crate exposes
  `xqasm` (`parse_xqasm`, `assemble_source`, `disassemble`) and
  `xqvm::Vm` to Python as `xqffi.asm` and `xqffi.vm`. `xqvm_py`
  consumes `xqffi.asm` only — its executor stays pure-Python so
