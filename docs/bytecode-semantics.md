...
| `0x07` | `ITER` | `reg: Register` | `[..., s, e] → [...]` | `read` — validates `reg` holds `VecInt` or `VecXqmx` | Pop `e` (`end_idx`), then `s` (`start_idx`). Validate that `reg` holds a `VecInt` or `VecXqmx`, then copy `vec[s..e]` into a new `Iter { elements, start_offset: s, index: 0 }` loop frame (the slice is duplicated so in-loop mutations of the source vec are not visible to `LVAL`). The next instruction's byte offset becomes `body_start`. Errors with `IndexOutOfBounds` if either index is negative, exceeds `vec.len()`, or if `s > e`. |
| `0x08` | `LIDX` | `reg: Register` | `[...] → [...]` | `write` — `reg ← Int(current)` for Range; `reg ← Int(start_offset + index)` for Iter | Copy the current loop *index* into `reg` as `Int`. For `Range` the values are themselves indices, so this is identical to `LVAL`. For `Iter` it returns the absolute position of the current element inside the *source* vec (`start_offset + index`), not the 0-based slice position. Errors with `NoActiveLoop` if no loop frame is active. |
