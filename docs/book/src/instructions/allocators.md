...
| `0x42` | `XQMX` | `reg: Register` | \\([\ldots, n, k] \to [\ldots]\\) | `write` | Pop \\(k\\), then \\(n\\). Allocate a discrete model with variable domain \\(\\{0, \ldots, k{-}1\\}\\). |
...
| `0x45` | `XSMX` | `reg: Register` | \\([\ldots, n, k] \to [\ldots]\\) | `write` | Pop \\(k\\), then \\(n\\). Allocate a discrete sample with \\(\text{values} = [0; n]\\). |

Samples hold a vector of variable assignments. The default value depends on the
domain: \\(0\\) for binary and discrete, \\(-1\\) for spin.
...
| Spin | \\(\\{-1, 1\\}\\) | `SQMX`, `SSMX` |
| Discrete(\\(k\\)) | \\(\\{0, \ldots, k{-}1\\}\\) | `XQMX`, `XSMX` |
