...
//! `python -m xqvm run`. Both produce an [`Outcome`] that is compared
//! against the vector's expected.json.
//!
...
/// Execute the vector against the Python reference VM (`python -m xqvm run`).
///
/// # Errors
...
pub fn run_python(vector: &Vector) -> Result<Outcome, String> {
    let python = std::env::var("XQUAD_CONFORMANCE_PYTHON").unwrap_or_else(|_| "python3".into());
    let xqvm_py_root = conformance_root().join("..").join("xqvm-py");

    let output = Command::new(&python)
        .args(["-m", "xqvm", "run", "--text", "--inputs"])
        .arg(vector.dir.join("inputs.json"))
        .args(["--outputs", &vector.inputs.output_slots.to_string()])
        .arg(vector.dir.join("program.xqasm"))
        .current_dir(&xqvm_py_root)
        .output()
        .map_err(|e| format!("failed to spawn {python} -m xqvm: {e}"))?;

    if !output.status.success() {
        return Err(format!(
            "python -m xqvm exited {}: stderr: {}",
            output.status,
...
pub enum Impl {
    ...
    Rust,
    /// Subprocess to `python -m xqvm run`.
    Python,
}
